...
#  replace_char = r'[）（%_■□·’‘》《』『“”［］]'
  ...
  for char in line:
    ...
    if char in punt_list:
      ...
      if len(match) == 0:
        chinese_reg  = re.compile(re_chinese)
        chinese_list = re.findall(chinese_reg, seg_line)
        seg_line = ''.join(chinese_list)
        if len(seg_line) >= 2:
...
def get_phn(sentence, phn_dict):
  ...
  '''
  nophn_fw = open("./nophn.txt", "w",encoding='utf-8', buffering=1024)
  phn_string = ""
    ...
    if word in phn_dict:
      ...
      for char in char_list:
        if char not in phn_dict:
          nophn_fw.write(char+'\r')
          continue
        phn_string = phn_string + " " + ' '.join(phn_dict[char])
  nophn_fw.close()
      ...
      with open(phn_output_file, 'w', buffering=1024) as phn_fw:
        for line in fr:
          ...
          for s in sentences:
            ...
            if len(phn_string) == 0:
              continue
            char_fw.write(char_string+'\n')
            phn_fw.write(phn_string+'\n')
...
def prepare_data(data_dir, phn_vocabulary_size, char_vocabulary_size):
  ...
  # 处理原始数据，生成音素文件
  pre_process_traindata(data_dir,'devdata')
  pre_process_traindata(data_dir,'traindata')
  

  train_path = data_dir+'traindata'
  dev_path = data_dir+'devdata'

  # 创建词汇表.
  char_vocab_path = os.path.join(data_dir, "vocab%d.char" % char_vocabulary_size)
  phn_vocab_path  = os.path.join(data_dir, "vocab%d.phn" % phn_vocabulary_size)
  create_vocabulary(char_vocab_path, train_path + ".char", char_vocabulary_size)
  create_vocabulary(phn_vocab_path, train_path + ".phn", phn_vocabulary_size)

  #后面就是把原始原料变成数字的表示， 每个单词改成数字
  char_train_ids_path = train_path + (".ids%d.char" % char_vocabulary_size)
  phn_train_ids_path  = train_path + (".ids%d.phn" % phn_vocabulary_size)
  data_to_token_ids(train_path + ".char", char_train_ids_path, char_vocab_path)
  data_to_token_ids(train_path + ".phn", phn_train_ids_path, phn_vocab_path )

  # 开发集数据
  char_dev_ids_path = dev_path + (".ids%d.char" % char_vocabulary_size)
  phn_dev_ids_path = dev_path + (".ids%d.phn" % phn_vocabulary_size)
  data_to_token_ids(dev_path + ".char", char_dev_ids_path, char_vocab_path)
  data_to_token_ids(dev_path + ".phn", phn_dev_ids_path, phn_vocab_path)

  return (phn_train_ids_path, char_train_ids_path,
          phn_dev_ids_path, char_dev_ids_path,
          phn_vocab_path, char_vocab_path)

if __name__=="__main__":
  prepare_data("../data/pingan/", 200, 4000)
