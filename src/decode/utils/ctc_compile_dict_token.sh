...
# This script compiles the lexicon and CTC tokens into FSTs. FST compiling
# slightly differs between the phoneme and character-based lexicons.
...
# Add probabilities to lexicon entries. There is in fact no point of doing this
# here since all the entries have 1.0. But utils/make_lexicon_fst.pl requires a
# probabilistic version, so we just leave it as it is.
