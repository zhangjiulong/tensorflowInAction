...
tf.app.flags.DEFINE_float('initial_learning_rate', 0.03, """学习率""")
...
def get_dev_data(dev_data_config):
  ...
  if not tf.gfile.Exists(FLAGS.eval_file):
    raise ValueError('Failed to find file: ' + FLAGS.eval_file)
  file_names = [FLAGS.eval_file]

  filename_queue = tf.train.string_input_producer(file_names)
  record_bytes = dev_data_config[0] * 4
  ...
  value_n = tf.reshape(value_n, [dev_data_config[0]])
...
def distort_inputs(data_config):
  ...
  filename_queue = tf.train.string_input_producer(data_paths, num_epochs=FLAGS.num_epochs)
...
def rnn(inputs, data_config):
  ...
  label_len = tf.to_int32(tf.reshape(label_len, [batch_size]))

  feature_area = tf.slice(inputs, [0, 2], [batch_size, data_config[1]])
  feature_area = tf.reshape(feature_area, [batch_size, -1, feature_cols])

  label_area = tf.slice(inputs, [0, 2 + data_config[1]],
  ...
  # [0,1,2,3,4,5,6,....,n]
  indices = [0]
  for i in range(1, data_config[2]):
  ...
  indice_matrix = tf.tile(batch_size_array, [data_config[2]])
  ...
  sparse_shape = [batch_size, data_config[2]]
...
def build_ctc(ctc_input, targets, seq_len):
  """构建模型的CTC部分.
