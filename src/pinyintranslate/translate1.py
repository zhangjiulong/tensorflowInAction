...
tf.app.flags.DEFINE_integer("en_vocab_size", 2000, "English vocabulary size.")
tf.app.flags.DEFINE_integer("fr_vocab_size", 10000, "French vocabulary size.")
tf.app.flags.DEFINE_string("data_dir", "data/", "Data directory")
tf.app.flags.DEFINE_string("train_dir", "traindata/", "Training directory.")
tf.app.flags.DEFINE_integer("max_train_data_size", 0,
                            "Limit on the size of training data (0: no limit).")
tf.app.flags.DEFINE_integer("steps_per_checkpoint", 600,
                            "How many training steps to do per checkpoint.")
tf.app.flags.DEFINE_boolean("decode", False,
                            "Set to True for interactive decoding.")
tf.app.flags.DEFINE_boolean("decode2", False,
                            "Set to True for interactive decoding.")
...
read_line_num_lasttime = 0
last_time_file = "./readnum.lasttime"

# We use a number of buckets and pad to the closest one for efficiency.
# See seq2seq_model.Seq2SeqModel for details of how they work.
#_buckets = [(5, 10), (10, 15), (20, 25), (40, 50),(70,75)]
_buckets = [(5, 5), (10, 10), (15, 15), (20, 20),(25,25),(30,30),(35,35),(40,40),(45,45),(50,50),(55,55),(60,60),(65,65),(70,70),(75,75)]
...
class BucketBuffer(object):
  ...
  def read_data(self, source_file, target_file):
    ...
    while source and target:
      ...
      #有时候重新加载模型， 读取的数据需要跳过前面已经读取过的
      if self.total_read_counter< read_line_num_lasttime:
        source, target = source_file.readline(), target_file.readline()
        #如果文件读完了， seek到最开头
        if not source or not target:
          source_file.seek(0,0)
      ...
      if self.total_read_counter % 10000 ==0:
        ...
        sys.stdout.flush()
        with open(last_time_file, 'w') as fw:
          fw.write(str(self.total_read_counter))
          fw.flush()
...
def create_model(session, forward_only):
  ...
  if ckpt and tf.gfile.Exists(ckpt.model_checkpoint_path):
    ...
    with open(last_time_file,'r') as f:
      l = f.readline()
      if l!='':
        read_line_num_lasttime = int(l) + 20000
      read_line_num_lasttime = read_line_num_lasttime % 138331031
      print("reload model, last time read number:", read_line_num_lasttime)
...
def train():
  ...
  with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    # every bucket has how many yuliao
    ...
    learn_rate_reset_flag = 0
    if learn_rate_reset_flag == 1:
      sess.run(model.learning_rate_reset_op)
      learn_rate_reset_flag = 0
...
def decode2():
  ...
  with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    sentence = sys.stdin.readline()
    while sentence:
      sentence = py.get_pinyin(sentence)
